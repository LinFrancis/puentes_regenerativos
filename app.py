"""
🧭 Puentes Regenerativos v4.0
Mapeando Puentes Regenerativos | Developed by Livlin.cl
Now with real Supabase connection.
"""
...
def get_supabase_db():
    """Return a SupabaseDB instance or None."""
...
_defaults = {"lang": DEFAULT_LANG, "logged_in": False, "user_id": None, "user_email": "",
             "page": "intro", "form_step": 0, "form_data": {}, "selected_profile": None,
             "admin_unlocked": False, "db_mode": "demo"}
...
def is_admin():
    if st.session_state.db_mode == "demo" and st.session_state.user_id == DEMO_USER_ID:
        return True
...
def do_supabase_login(email, password):
    """Login with Supabase Auth and switch DB to real Supabase."""
    client = get_supabase_client()
    if not client:
    ...
    try:
        ...
        st.session_state.db = get_supabase_db()
        return True
    except Exception as e:
        st.error(f"{t('auth.login_error', lang)}: {e}")
        return False

def do_supabase_register(email, password):
...
def do_demo_login():
    """Login in demo mode with in-memory DB."""
    st.session_state.logged_in = True
...
with st.sidebar:
    ...
    new_lang = st.selectbox(t("language_selector", lang), list(LANG_LABELS.keys()),
    ...
    if not st.session_state.logged_in:
        ...
        if HAS_SUPABASE:
            ...
            with c1:
                if st.button(t("auth.login_btn", lang), use_container_width=True, type="primary"):
                    if do_supabase_login(auth_email, auth_pwd):
                        st.rerun()
            with c2:
                if st.button(t("auth.register_btn", lang), use_container_width=True):
                    do_supabase_register(auth_email, auth_pwd)
            st.divider()
            st.caption("— " + ("o prueba con datos de ejemplo" if lang == "es" else "or try with sample data" if lang == "en" else "o prova amb dades d'exemple") + " —")

        # ── Demo mode (always available) ──
        if st.button(t("auth.demo_login", lang), use_container_width=True, type="primary" if not HAS_SUPABASE else "secondary"):
            do_demo_login(); st.rerun()

        if not HAS_SUPABASE:
            st.caption("ℹ️ " + ("Supabase no configurado. Configura SUPABASE_URL y SUPABASE_KEY en los Secrets para habilitar login con email." if lang == "es"
                                 else "Supabase not configured. Set SUPABASE_URL and SUPABASE_KEY in Secrets to enable email login." if lang == "en"
                                 else "Supabase no configurat. Configura SUPABASE_URL i SUPABASE_KEY als Secrets."))
    else:
        # Show connection mode
        mode_label = "🟢 Supabase" if st.session_state.db_mode == "supabase" else "🟡 Demo"
        ...
        nav_items = [
            ...
            ("map", t("nav.map", lang)), ("profiles", t("nav.profiles", lang)),
            ("network", t("nav.network", lang)), ("forum", t("nav.forum", lang)),
        ...
        if st.button(t("nav.new_profile", lang), use_container_width=True):
            st.session_state.form_data = {}; st.session_state.form_step = 0; nav("new_profile"); st.rerun()
...
if not st.session_state.logged_in:
    ...
    with c2:
        ...
        if lang == "es": st.markdown("Una plataforma para mapear actores que trabajan en regeneración a nivel global, visualizar los puentes entre ellos, y analizar el estado de la red regenerativa.")
        elif lang == "en": st.markdown("A platform to map actors working in regeneration globally, visualize the bridges between them, and analyze the state of the regenerative network.")
        else: st.markdown("Una plataforma per mapejar actors que treballen en regeneració a nivell global.")
...
# ══════════════════════════════════════════════════════════════
db = st.session_state.db  # Refresh reference after potential switch
...
# ══════════════════════════════════════════════════════════════
# ALL PAGES (same as v3 — just the page functions)
# ══════════════════════════════════════════════════════════════
...
""")
        ...
        st.markdown("""A **regenerative bridge** is both the actor and the connections between actors working for ecosocial regeneration. The network is a **living system** where more connections = greater regeneration.

This platform lets you **map**, **visualize**, **analyze**, **measure**, and **connect** regenerative actors globally.""")
    else:
        st.markdown("Un **pont regeneratiu** és tant l'actor com les connexions. Mapejar, visualitzar, analitzar, mesurar i connectar actors regeneratius.")

    st.markdown("---")
    st.markdown("### 🌀 " + ("Dimensiones de Regeneración (Mandala GEN)" if lang == "es" else "Regeneration Dimensions (GEN Mandala)" if lang == "en" else "Dimensions de Regeneració"))
    if lang == "es": st.markdown("Cada actor identifica en qué **dimensiones** trabaja, basadas en el Mandala de Sustentabilidad de la Red Global de Ecoaldeas (GEN):")
    cols = st.columns(5)
    ...
    st.markdown("---")
    st.markdown("### ⚡ " + ("Ejes de Acción (COP30 Action Agenda)" if lang == "es" else "Action Axes (COP30)" if lang == "en" else "Eixos d'Acció (COP30)"))
    if lang == "es": st.markdown("**60 acciones concretas** organizadas en 6 ejes temáticos de la Agenda de Acción Climática COP30:")
    for ax in axes_data:
        with st.expander(f"{ax['icon']} {ax['name']} ({len(ax['actions'])})"):
            for a in ax["actions"]: st.markdown(f"- {a['name']}")

    st.markdown("---")
    non_ghost = [p for p in profiles if not p.get("es_fantasma")]
    st.markdown("### 📊 " + ("Estado actual de la red" if lang == "es" else "Network status" if lang == "en" else "Estat de la xarxa"))
    c1, c2, c3, c4 = st.columns(4)
    with c1: metric(len(non_ghost), "Actores")
    with c2: metric(len(connections), "Puentes")
    with c3: metric(len(set(p.get("pais") for p in non_ghost if p.get("pais"))), "Países")
    with c4:
        avg = sum(p.get("indice_puentes", 0) for p in non_ghost) / max(len(non_ghost), 1)
        metric(f"{avg:.0f}/100", "Índice promedio")

    # Connection mode indicator
    if st.session_state.db_mode == "supabase":
        st.success("🟢 " + ("Conectado a Supabase — los datos son reales y persistentes." if lang == "es" else "Connected to Supabase — data is real and persistent." if lang == "en" else "Connectat a Supabase — dades reals."))
    else:
        st.warning("🟡 " + ("Modo demo — los datos son de ejemplo y no se guardan entre sesiones." if lang == "es" else "Demo mode — sample data, not persisted between sessions." if lang == "en" else "Mode demo — dades d'exemple."))

    st.markdown("---")
    st.markdown("#### " + ("¿Cómo empezar?" if lang == "es" else "How to start?" if lang == "en" else "Com començar?"))
    if lang == "es":
        st.markdown("""1. Explora el **Mapa** para ver los actores y sus conexiones
2. Revisa los **Perfiles** para conocer cada actor en detalle
3. Visita la **Red** para ver el grafo de puentes regenerativos
...
def render_map_page():
    ...
    if lang == "es": st.markdown("**Marcadores azules** = perfiles, **líneas** = puentes, **círculos naranjas** = territorios de impacto.")
    elif lang == "en": st.markdown("**Blue markers** = profiles, **lines** = bridges, **orange circles** = impact territories.")
    ...
    for p in filtered:
        if p.get("lat") and p.get("lon"):
            ...
            folium.Marker([p["lat"], p["lon"]], popup=folium.Popup(popup, max_width=250), tooltip=p["nombre"], icon=folium.Icon(color=color, icon="leaf", prefix="fa")).add_to(m)
    ...
    for c in connections:
        ...
        if s and tg and s.get("lat") and tg.get("lat"):
            folium.PolyLine([[s["lat"], s["lon"]], [tg["lat"], tg["lon"]]], weight=c.get("intensidad", 1) * 0.8, color="#1976D2", opacity=0.5, dash_array="5" if c.get("es_externa") else None).add_to(m)
    ...
    with tab1:
        if lang == "es": st.caption("País > Región > Ciudad > Actor. Tamaño = Índice Puentes.")
        import plotly.express as px; import pandas as pd
        data = [{"pais": p.get("pais", "?"), "region": p.get("region", "?"), "ciudad": p.get("ciudad", "?"), "nombre": p["nombre"], "indice": max(p.get("indice_puentes", 1), 1)} for p in filtered if not p.get("es_fantasma")]
        if data:
            fig = px.treemap(pd.DataFrame(data), path=["pais", "region", "ciudad", "nombre"], values="indice", color="indice", color_continuous_scale=["#E3F2FD", "#1565C0"])
            fig.update_layout(font_family="Montserrat", margin=dict(t=30, l=10, r=10, b=10)); st.plotly_chart(fig, use_container_width=True)
    with tab2:
        if lang == "es": st.caption("Proporción por dimensión del Mandala GEN. Más equilibrado = mayor diversidad sistémica.")
        ...
        fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100])), font_family="Montserrat"); st.plotly_chart(fig, use_container_width=True)
    with st.expander(f"🧮 {t('index.title', lang)} — Fórmula"):
        st.markdown(get_formula_explanation(lang))


def render_profiles_page():
    st.markdown(f"## {t('nav.profiles', lang)}")
    if lang == "es": st.markdown("Directorio de actores ordenados por Índice Puentes Regenerativos.")
    search = st.text_input("🔍", placeholder="Buscar..." if lang == "es" else "Search...")
    at = get_actor_types(lang)
    filtered = [p for p in profiles if not p.get("es_fantasma")]
    if search:
        sl = search.lower()
        filtered = [p for p in filtered if sl in p.get("nombre", "").lower() or sl in p.get("pais", "").lower() or sl in (p.get("descripcion", "") or "").lower()]
...
def render_profile_detail():
    ...
    if st.button("← " + ("Volver" if lang == "es" else "Back")): nav("profiles"); st.rerun()


def render_network_page():
    st.markdown(f"## {t('network_view.title', lang)}")
    if lang == "es": st.markdown("Cada **nodo** = actor, cada **línea** = puente. Tamaño = índice + conexiones. Color = intensidad del índice.")
    import networkx as nx; import plotly.graph_objects as go
    G = nx.Graph(); pmap = {p["id"]: p for p in profiles}
    ...
    fig = go.Figure([go.Scatter(x=ex, y=ey, mode="lines", line=dict(width=1, color="#90CAF9"), hoverinfo="none"),
        go.Scatter(x=nx_, y=ny_, mode="markers+text", text=[pmap.get(n, {}).get("nombre", "?")[:18] for n in G.nodes()], textposition="top center", textfont=dict(size=9), hovertext=nt_, hoverinfo="text",
...
def render_new_profile():
    ...
    if lang == "es": st.markdown("Completa los 6 pasos para registrar tu perfil regenerativo.")
    ...
    if cur == 0:
        ...
        st.caption("💡 " + ("Busca las coordenadas en Google Maps." if lang == "es" else "Look up coordinates on Google Maps."))
    elif cur == 2:
        ...
        for tr in fd.get("pending_territories", []): st.markdown(f"📍 {tr['ciudad']}, {tr['pais']}")
    st.markdown("")
...
def render_my_profiles():
    ...
    for p in my:
        ...
        if st.button(f"👁️ {p['nombre']}", key=f"s_{p['id']}"): st.session_state.selected_profile = p["id"]; nav("profile_detail"); st.rerun()


def render_forum():
    st.markdown(f"## {t('forum.title', lang)}")
    if lang == "es": st.markdown("Espacio para compartir experiencias y fortalecer conexiones.")
    with st.expander(f"➕ {t('forum.new_thread', lang)}"):
        ...
        if st.button(t("forum.post", lang), key="pt"):
            if nc.strip():
                autor = st.session_state.user_email or "Demo User"
                db.add_message({"user_id": st.session_state.user_id, "contenido": nc, "parent_id": None, "autor_nombre": autor}); st.rerun()
    for msg in db.get_messages(parent_id=None):
        ...
        with st.expander(f"↩️ {t('forum.reply', lang)}"):
            ...
            if st.button(t("forum.post", lang), key=f"pr_{msg['id']}"):
                if rc.strip():
                    autor = st.session_state.user_email or "Demo User"
                    db.add_message({"user_id": st.session_state.user_id, "contenido": rc, "parent_id": msg["id"], "autor_nombre": autor}); st.rerun()


def render_bibliography():
    st.markdown(f"## {t('bibliography.title', lang)}")
    if lang == "es": st.markdown("Referencias sobre regeneración, permacultura, diseño regenerativo y acción climática.")
    for b in get_bibliography():
        st.markdown(f"### 📖 {b['titulo']}"); st.markdown(f"**{b['autor']}**")
...
def render_admin():
    ...
    if st.session_state.db_mode != "demo" and ADMIN_PASSWORD and not st.session_state.admin_unlocked:
        ...
        return
    if st.session_state.db_mode == "demo": st.info(t("admin.demo_admin_note", lang))
    c1, c2, c3 = st.columns(3)
    ...
    st.markdown("---")
    st.markdown(f"### {t('admin.download_title', lang)}"); st.markdown(t("admin.download_desc", lang))
    try:
